...
if($orderswitcher->read("qjump") == 0) $withoutJump = true;//без перемычек
else $withoutJump = false;//с перемычками


  $scheme->qPos = $orderswitcher->read("qpos");//получение количества положений
  $scheme->qCont = $orderswitcher->read("qcont");//получение количества контактов
  $scheme->closDiag = $orderswitcher->read("closdiag");//получение диаграммы замыканий
	if($orderswitcher->read("board") == 0) $scheme->presMark = 0;
	else $scheme->presMark = 1;//получение наличия маркировки
  $scheme->mark = $orderswitcher->read("mark");//получение маркировки
  $scheme->qJump = $orderswitcher->read("qjump");//получение количества пермычек
  $scheme->jump = $orderswitcher->read("jump");//получение перемычек
  //$scheme->jump .= "#";//добавление фиктивного # что бы в cookie не был удален последний пробел
...
outimg: Header("Content-type: image/jpeg");
  ...
  //imagegif($canvaScheme, $_SERVER["REMOTE_ADDR"]."/".$orderswitcher->read"npo"].".gif");//запись изображения в файл
