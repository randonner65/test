...
else {//если схему выводит конструктор переключателя
$scheme = new  Schemas();//создание объекта класса Schemas
  $scheme->qpos = $_GET["qpos"];//получение количества положений
  $scheme->qcont = $_GET["qcont"];//получение количества контактов
  $scheme->closdiag = $_GET["closdiag"];//получение диаграммы замыканий
	if($_GET["board"] == 0) $scheme->presMark = 0;
	else $scheme->pmark = 1;//получение наличия маркировки
  $scheme->mark = $_GET["mark"];//получение маркировки
  $scheme->qjump = $_GET["qjump"];//получение количества пермычек
  $scheme->jump = $_GET["jump"];//получение перемычек
}
 $cellSize = 30;//размер ячейки таблицы схемы
 if(!$scheme->checkExistScheme($name) && isset($_GET['number'])) {
 //echo $name;echo "</br>";
  ...
  //imageTtfText($canvaScheme, $cellSize/2.5, 0, 30, 10, $color, "../../fonts/arial.ttf", "jkohoihjioj");
...
$numCont = $scheme->qcont;//количество контактных групп (для доступа внутри функций)


//Определение максимальной длинны маркировки
$maxlength = 0;
 for ($i=0; $i<$scheme->qpos; $i++)
	if(mb_strlen($markArr[$i],"UTF-8") > $maxlength)
		$maxlength = mb_strlen($markArr[$i],"UTF-8");
...
//Запись массива перемычек
 ...
 $index = 0;
	for ($n=0; $n<$scheme->qjump; $n++){ //n - номер перемычки
		...
		$jumpArr[1][$n] = (int) getWord($scheme->jump);//номер второго контакта перемычки
	}
$maxNTrace=0;
//определение ширины первого столбца в зависимости от топологии перемычек	
if(!$withoutJump) require_once "../blocks/widthfirstcol.php";
...
$widthFirstCol = $cellSize*4+2*($maxNTrace-1)*$cellSize/3;
  ...
  ImageColorTransparent($canvaScheme , $color);//создание прозрачного фона
//Вывод таблицы
...
imageSetThickness($canvaScheme, 2);//толщина линий
 ...
 //imageTtfText($canvaScheme, $cellSize/2.5, 0, 30, 30, $color, "../../fonts/arial.ttf",  $name); 
  for ($i=0; $i<$scheme->qpos; $i++){//вывод вертикальных линий
  ...
  if($j%2 == 0 && $j !=0) imageSetThickness($canvaScheme, 3);//толщина линий
  else imageSetThickness($canvaScheme, 1);//толщина линий
  imageLine($canvaScheme, 0, $heightTxt+$heightMarkPos+$j*$cellSize, $scheme->qpos*$cellSize+$widthFirstCol, $heightTxt+$heightMarkPos+$j*$cellSize, $color);
   imageSetThickness($canvaScheme, 1);//толщина линий
...
//Вывод замыкающих положений
 for ($j=0; $j<$numCont; $j++)
   for ($i=0; $i<$scheme->qpos; $i++)
   {
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "x") outCross(); 
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "a") outOverlapLeft();
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "b") outOverlapRight();
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "c") outOverlapBoth();
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "h") outPulseRight();
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "g") outPulseLeft(); 
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "u") outCrossRight(); 
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "v") outCrossLeft();
     if ($scheme->closdiag[$j*$scheme->qpos+$i] == "w") outHorLine(); 
   }
...
for ($n=0; $n<$scheme->qjump; $n++){
 ...
 Header("Content-type: image/jpeg");
  ...
  if($_GET["flag"] == 1) imagegif($canvaScheme, '../../images/passport/'.$name.'.jpg');
  else imagegif($canvaScheme);
  imageDestroy($canvaScheme);
...
//imageLine($canvaScheme, 0,0,20,20, $color);
